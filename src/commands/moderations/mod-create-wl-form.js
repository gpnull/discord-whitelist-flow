...
// module.exports = {
//   name: "create-wl-form",
//   description: "Tạo form đăng ký whitelist cho người mới.",
...
module.exports = {
  ...
  callback: async (client, interaction) => {
    try {
      ...
      const row = createActionRow(createAcceptRegulationButton());

      const reply = await channel.send({ embeds: [embed], components: [row] });
      const collector = createCollector(reply);

      collector.on("collect", async (interaction) => {
        if (interaction.customId === "acceptRegulationBtn") {
          await handleAcceptRegulationButton(client, interaction);
        }
      });
...
function createAcceptRegulationButton() {
...
function createCollector(reply) {
  return reply.createMessageComponentCollector({
    componentType: ComponentType.Button,
  });
}

async function handleAcceptRegulationButton(client, interaction) {
  let discordSaved = false;
  await interaction.deferReply({ ephemeral: true });
  ...
  if (!discordSaved) {
    await createDiscordDataIfNeeded(
      client,
      checkDiscordData.pageDiscordData,
      interaction.user.id,
      interaction.user.username
...
async function createDiscordDataIfNeeded(
  client,
  pageDiscordData,
  userId,
  username
  ...
  const checkDiscordData = await discordData.callback(client);
  const discordSaved = checkDiscordData.idDiscordRegistered.some((data) =>
    data.includes(userId)
  );

  if (!discordSaved) {
    await createDiscordData.callback(client, pageDiscordData, userId, username);
  }
}
